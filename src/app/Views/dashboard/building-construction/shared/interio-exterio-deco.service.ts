...
export class InterioExterioDecoService {
  ...
  constructor(private afs: AngularFirestore, private authService: AuthService) {
    ...
    this.dbcollections = this.afs.doc(
      `Interio-exterio-decoration/${this.CurrentUser.uid}`
    );

    //setting up form with formgroup
    this.form = new FormGroup({
      occupation: new FormControl("interio-exterio-deco", Validators.required),
  ...
  initializeFormGroup() {
    this.form.setValue({
      occupation: "interio-exterio-deco",
      country: "",
      state: "",
      city: "",
      phonenumber: ""
